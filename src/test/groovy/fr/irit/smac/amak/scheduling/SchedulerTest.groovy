...
class SchedulerTest extends Specification {
	...
	def "When step is called and the scheduler is in state #initialState, then the executor should be run #runTimes and the next state should be #nextState"() {
		...
		!scheduler.stateLock.locked
		stateChangesCount * consumer.accept(_)

		where:
		initialState                          | runTimes | nextState                             | stateChangesCount
		Scheduler.SchedulerState.IDLE         | 1        | Scheduler.SchedulerState.PENDING_STOP | 1
		Scheduler.SchedulerState.PENDING_STOP | 0        | Scheduler.SchedulerState.PENDING_STOP | 0
		Scheduler.SchedulerState.RUNNING      | 0        | Scheduler.SchedulerState.RUNNING      | 0
	}
	...
	def "When stop is called and the scheduler is in state #initialState, then the executor should the next state should be #nextState"() {
		...
		where:
		initialState                          | nextState                             | stateChangesCount
		Scheduler.SchedulerState.IDLE         | Scheduler.SchedulerState.IDLE         | 0
		Scheduler.SchedulerState.PENDING_STOP | Scheduler.SchedulerState.PENDING_STOP | 0
		Scheduler.SchedulerState.RUNNING      | Scheduler.SchedulerState.PENDING_STOP | 1
	}
